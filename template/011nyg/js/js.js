...
$.fn.scrollShow=function(){
    var begin=0, 
    rollDom = this.find('.scroll'),
    wideRoll = parseInt(rollDom.width()),
    setCounter=null;
    this.append(rollDom.clone(true));
    setCounter=setInterval(function () {
        begin -= 2;
        rollDom.css({ 'margin-left': begin + 'px' })
        if (-begin >= wideRoll) {
            begin = 0;
        }
    },20);
...
$(function (){
  ...
  if( winWidth <= 840 ){
    console.log( winWidth );
		$(".nav a.nav_parent").on({
    	click : function(){
          if($(this).next(".sub_nav").height() === 0){				// 如子菜单 不显示，则点击显示
          	$(".nav .sub_nav").css({      // 先隐藏所有的子菜单
              "height": "0"				
            });
        	$(this).next(".sub_nav").css({		// 使this的下一个 子菜单显示
              "display": "block",
              "position": "absolute",
              "background": "#af1700",
              "top": "38px",
          	...
          	$("#nav").css({				// 增加 导航栏的 高度， 使子菜单 显示
        		"height":"66px"
        	});
    		$(".main.left_spare").css({			// 增加广告相应的padding 使其不被遮挡
            	"padding-top": "120px"
            })
          } 
          else {
			$(".nav .sub_nav").css({      // 隐藏所有的子菜单
              "height": "0"				
            });
          	$("#nav").css({				// 初始化 导航栏的 高度， 使子菜单 显示
        		"height":"38px"
        	});
    		$(".main.left_spare").css({			// 初始化 广告相应的padding 使其不被遮挡
            	"padding-top": "89px"
            })
